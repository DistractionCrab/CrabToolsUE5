...
TSharedRef<FStateMachineGraphDragConnection> FStateMachineGraphDragConnection::New(const TSharedRef<SGraphPanel>& GraphPanel, const FDraggedPinTable& DraggedPins)
{
	TSharedRef<FStateMachineGraphDragConnection> Operation = MakeShareable(new FStateMachineGraphDragConnection(GraphPanel, DraggedPins));
	Operation->Construct();
...
void FStateMachineGraphDragConnection::HoverTargetChanged()
{
	...
	else if (UEdStateNode* TargetNodeObj = Cast<UEdStateNode>(GetHoveredNode()))
	{
		TArray<UEdGraphPin*> ValidSourcePins;
		ValidateGraphPinList(/*out*/ ValidSourcePins);
...
FStateMachineGraphDragConnection::FStateMachineGraphDragConnection(const TSharedRef<SGraphPanel>& GraphPanelIn, const FDraggedPinTable& DraggedPinsIn)
...
FReply FStateMachineGraphDragConnection::DroppedOnPanel(const TSharedRef< SWidget >& Panel, FVector2D ScreenPosition, FVector2D GraphPosition, UEdGraph& Graph)
{
	// Gather any source drag pins
...
void FStateMachineGraphDragConnection::ValidateGraphPinList(TArray<UEdGraphPin*>& OutValidPins)
{
