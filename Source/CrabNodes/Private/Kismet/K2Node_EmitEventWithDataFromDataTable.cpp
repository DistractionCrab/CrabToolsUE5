...
#define LOCTEXT_NAMESPACE "K2Node_EmitEventWithDataFromDataTable"

namespace EmitEventWithDataFromDataTableHelper
{
...
UK2Node_EmitEventWithDataFromDataTable::UK2Node_EmitEventWithDataFromDataTable(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	NodeTooltip = LOCTEXT("NodeTooltip", "Emits an event to a State Machine from a Data Table.");
}

void UK2Node_EmitEventWithDataFromDataTable::AllocateDefaultPins()
{
	...
	UEdGraphPin* StateMachinePin = CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Object, UEventListenerInterface::StaticClass(), EmitEventWithDataFromDataTableHelper::StateMachinePinName);
	SetPinToolTip(*StateMachinePin, LOCTEXT("StateMachinePinDescription", "The EventListener to want to send an event to"));

	// Add DataTable pin
	UEdGraphPin* DataTablePin = CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Object, UDataTable::StaticClass(), EmitEventWithDataFromDataTableHelper::DataTablePinName);
	SetPinToolTip(*DataTablePin, LOCTEXT("DataTablePinDescription", "The DataTable you want to retreive a row from"));

	// Row Name pin
	UEdGraphPin* RowNamePin = CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Name, EmitEventWithDataFromDataTableHelper::RowNamePinName);
	SetPinToolTip(*RowNamePin, LOCTEXT("RowNamePinDescription", "The name of the row to retrieve from the DataTable"));

	// Add Data Pin
	UEdGraphPin* DataPin = CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Object, UObject::StaticClass(), EmitEventWithDataFromDataTableHelper::DataPinName);
	SetPinToolTip(*DataPin, LOCTEXT("DataPinDescription", "The data to be passed with the event."));

	Super::AllocateDefaultPins();
}

void UK2Node_EmitEventWithDataFromDataTable::SetPinToolTip(UEdGraphPin& MutatablePin, const FText& PinDescription) const
{
...
void UK2Node_EmitEventWithDataFromDataTable::RefreshRowNameOptions()
{
...
void UK2Node_EmitEventWithDataFromDataTable::OnDataTableRowListChanged(const UDataTable* DataTable)
{
...
void UK2Node_EmitEventWithDataFromDataTable::ReallocatePinsDuringReconstruction(TArray<UEdGraphPin*>& OldPins) 
{
...
void UK2Node_EmitEventWithDataFromDataTable::GetMenuActions(FBlueprintActionDatabaseRegistrar& ActionRegistrar) const
{
...
FText UK2Node_EmitEventWithDataFromDataTable::GetMenuCategory() const
{
	return FEditorCategoryUtils::GetCommonCategory(FCommonEditorCategory::Utilities);
}

bool UK2Node_EmitEventWithDataFromDataTable::IsConnectionDisallowed(const UEdGraphPin* MyPin, const UEdGraphPin* OtherPin, FString& OutReason) const
{
...
void UK2Node_EmitEventWithDataFromDataTable::PinDefaultValueChanged(UEdGraphPin* ChangedPin) 
{
	if (ChangedPin && ChangedPin->PinName == EmitEventWithDataFromDataTableHelper::DataTablePinName)
	{
...
FText UK2Node_EmitEventWithDataFromDataTable::GetTooltipText() const
{
	return NodeTooltip;
}

UEdGraphPin* UK2Node_EmitEventWithDataFromDataTable::GetThenPin()const
{
	const UEdGraphSchema_K2* K2Schema = GetDefault<UEdGraphSchema_K2>();
...
UEdGraphPin* UK2Node_EmitEventWithDataFromDataTable::GetDataTablePin(const TArray<UEdGraphPin*>* InPinsToSearch /*= NULL*/) const
{
	...
	{
		if (TestPin && TestPin->PinName == EmitEventWithDataFromDataTableHelper::DataTablePinName)
		{
			Pin = TestPin;
			break;
		}
...
UEdGraphPin* UK2Node_EmitEventWithDataFromDataTable::GetRowNamePin() const
{
	UEdGraphPin* Pin = FindPinChecked(EmitEventWithDataFromDataTableHelper::RowNamePinName);
	check(Pin->Direction == EGPD_Input);
	return Pin;
}

UEdGraphPin* UK2Node_EmitEventWithDataFromDataTable::GetStateMachinePin() const
{
	UEdGraphPin* Pin = FindPinChecked(EmitEventWithDataFromDataTableHelper::StateMachinePinName);
	check(Pin->Direction == EGPD_Input);
	return Pin;
}

UEdGraphPin* UK2Node_EmitEventWithDataFromDataTable::GetDataPin() const
{
	UEdGraphPin* Pin = FindPinChecked(EmitEventWithDataFromDataTableHelper::DataPinName);
	check(Pin->Direction == EGPD_Input);
	return Pin;
}

FText UK2Node_EmitEventWithDataFromDataTable::GetNodeTitle(ENodeTitleType::Type TitleType) const
{
	if (TitleType == ENodeTitleType::MenuTitle)
	{
		return LOCTEXT("ListViewTitle", "Emit Event With Data From DataTable");
	}
	else if (UEdGraphPin* DataTablePin = GetDataTablePin())
	{
		if (DataTablePin->LinkedTo.Num() > 0)
		{
			return NSLOCTEXT("K2Node", "DataTable_Title_Unknown", "Emit Event With Data From DataTable");
		}
		else if (DataTablePin->DefaultObject == nullptr)
		{
			return NSLOCTEXT("K2Node", "DataTable_Title_None", "Emit Event With Data From DataTable Row NONE");
		}
		else if (CachedNodeTitle.IsOutOfDate(this))
		{
			...
			FText LocFormat = NSLOCTEXT("K2Node", "DataTable", "Emit Event With Data From DataTable Row {DataTableName}");
...
void UK2Node_EmitEventWithDataFromDataTable::ExpandNode(class FKismetCompilerContext& CompilerContext, UEdGraph* SourceGraph)
{
	...
	{
		CompilerContext.MessageLog.Error(*LOCTEXT("EmitEventWithDataFromDataTableNoDataTable_Error", "EmitEventWithDataFromDataTable must have a StateMachine specified.").ToString(), this);
		// we break exec links so this is the only error we get
		BreakAllNodeLinks();
		return;
	}
	...
	UEdGraphPin* DataInPin = EmitEventFromDataTableFunction->FindPinChecked(TEXT("Data"));
...
FSlateIcon UK2Node_EmitEventWithDataFromDataTable::GetIconAndTint(FLinearColor& OutColor) const
{
...
void UK2Node_EmitEventWithDataFromDataTable::PostReconstructNode()
{
...
void UK2Node_EmitEventWithDataFromDataTable::EarlyValidation(class FCompilerResultsLog& MessageLog) const
{
...
void UK2Node_EmitEventWithDataFromDataTable::PreloadRequiredAssets()
{
...
void UK2Node_EmitEventWithDataFromDataTable::NotifyPinConnectionListChanged(UEdGraphPin* Pin)
{
